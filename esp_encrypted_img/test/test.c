...
static int32_t fetch_nvs_i_value(nvs_handle_t nvs_ota_resumption_handle)
{
    int32_t i = 0;

    // Check if the key is not found, meaning the value is not yet initialized
    esp_err_t err = nvs_get_i32(nvs_ota_resumption_handle, "key_i", &i);
...
static esp_err_t save_ota_state_to_nvs(esp_decrypt_handle_t decrypt_handle, nvs_handle_t nvs_handle, int32_t i)
{
    if (decrypt_handle == NULL || nvs_handle == 0) {
    ...
    ESP_RETURN_ON_ERROR(nvs_set_i32(nvs_handle, "key_i", i), TAG, "Failed to set i");

    ESP_RETURN_ON_ERROR(esp_encrypted_img_decrypt_handle_to_nvs(decrypt_handle, &buffer, &buffer_len), TAG, "Failed to serialize decrypt handle");

    err = nvs_set_blob(nvs_handle, "decrypt_handle", buffer, buffer_len);
    free(buffer);
    ESP_RETURN_ON_ERROR(err, TAG, "Failed to save decrypt handle to NVS");
    
    ESP_LOGI(TAG, "Saving state");
    ESP_RETURN_ON_ERROR(nvs_commit(nvs_handle), TAG, "Failed to commit NVS");

    ESP_LOGD(TAG, "OTA state and decrypt handle saved to NVS successfully");
    return ESP_OK;
}


static esp_decrypt_handle_t load_decrypt_handle_from_nvs(nvs_handle_t nvs_handle)
{
    if (nvs_handle == 0) {
    ...
    esp_err_t err = nvs_get_blob(nvs_handle, "decrypt_handle", NULL, &required_size);
    if (err == ESP_ERR_NVS_NOT_FOUND) {
        ESP_LOGI(TAG, "No decrypt handle found in NVS");
        return NULL;
    } else if (err != ESP_OK) {
        ESP_LOGE(TAG, "Error in getting decrypt handle size from NVS: %s", esp_err_to_name(err));
        return NULL;
    }
    ...
    if (buffer == NULL) {
        ESP_LOGE(TAG, "Failed to allocate memory for decrypt handle");
        return NULL;
    }

    // Read the serialized data from NVS
    err = nvs_get_blob(nvs_handle, "decrypt_handle", buffer, &required_size);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Failed to read decrypt handle from NVS: %s", esp_err_to_name(err));
        free(buffer);
        return NULL;
    }

    // Deserialize the data into the decrypt handle
    esp_decrypt_handle_t decrypt_handle = NULL;
    err = esp_encrypted_img_decrypt_handle_from_nvs(buffer, required_size, &decrypt_handle);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Failed to deserialize decrypt handle: %s", esp_err_to_name(err));
        free(buffer);
        return NULL;
    }
    ...
    ESP_LOGD(TAG, "Decrypt handle loaded from NVS successfully");
...
TEST_CASE("Sending 1 byte data at once", "[encrypted_img]")
{
    ...
    esp_decrypt_handle_t ctx = NULL;
    ctx = load_decrypt_handle_from_nvs(nvs_ota_resumption_handle);
    int32_t i = fetch_nvs_i_value(nvs_ota_resumption_handle);
    
    if (ctx == NULL) {
        // If decrypt handle wasn't loaded from NVS or ota_image_written is not greater than header size, start a new one
        ESP_LOGI(TAG, "Fresh OTA");
    ...
    do {
        ...
        args->data_in_len = 1;
        err = esp_encrypted_img_decrypt_data(ctx, args);
        ESP_LOGI(TAG, "i = %u err = %d", (unsigned int)i, err);
        if (err == ESP_FAIL) {
            printf("ESP_FAIL ERROR\n");
            break;
        }
        if( i >= 10024) {
            save_ota_state_to_nvs(ctx, nvs_ota_resumption_handle, i);
            }      
        if( i == 10026) {
            esp_restart();
            }   
